...
def main() -> None:
    ...
    parser.add_argument("--checkpoint", required=True); parser.add_argument("--predictions", required=True)
    parser.add_argument("--split", required=True); parser.add_argument("--output-dir", required=True)
    parser.add_argument("--methods", nargs="+", default=["gradcam"]); parser.add_argument("--samples-per-group", type=int, default=5); parser.add_argument("--threshold", type=float, required=True)
    args = parser.parse_args(); output = Path(args.output_dir); output.mkdir(parents=True, exist_ok=True)
    state = torch.load(args.checkpoint, map_location="cpu", weights_only=False); config = state["configuration"]
    model_config, training = config["model"], config["training"]; model = create_model(model_config["name"], pretrained=False); model.load_state_dict(state["model_state_dict"]); model.eval()
    _, transform = _transforms(int(training["image_size"]), model_config.get("preprocessing", "imagenet"))
    predictions = pd.read_csv(args.predictions); selected = select_cases(predictions.loc[predictions["split"] == args.split], args.threshold, args.samples_per_group); rows = []
    for index, case in selected.iterrows():
        image_path = Path(args.root) / Path(case["image_path"]); image = transform(Image.open(image_path).convert("RGB"))
        for method in args.methods:
            attribution = generate_attribution(model, image, method); filename = f"{index}_{case['outcome_group']}_{method}.png"; save_explanation_figure(image, attribution, f"{case['outcome_group']} p={case['probability']:.3f}", output / filename)
